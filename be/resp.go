...
// Send an HTTP response with the given code and no data: see status defaults
// in `resp.SendData`
func (resp *Resp) Send(code RespCode) error {
	return resp.SendData(code, nil)
}

// Sends an HTTP response witht he given code and data; the status defaults to
// 400 for the error codes, 200 for success codes, and the code itself if it's
// equal to an HTTP status.
func (resp *Resp) SendData(code RespCode, data interface{}) error {
	if code >= 2000 {
