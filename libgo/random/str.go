...
// Base64Str generates a random base64 string with length of n.
func Base64Str(n int) string {
	return String(n, "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ+/")
}

// Base62Str generates a random base62 string with length of n.
func Base62Str(s int) string {
	return String(s, "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
}

// HexStr generates a random hex string with length of n.
// e.g: 67aab2d956bd7cc621af22cfb169cba8
func HexStr(n int) string {
	return hex.EncodeToString(Bytes(n))
}

// list of default letters that can be used to make a random string when
// calling String function with no letters provided.
var defLetters = []rune("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")

// String generates a random string using only letters provided in the letters
// parameter if user ommit letters parameters, this function will use
// defLetters instead.
